...
begin
	...
	(
		G_ID			bigint identity(1,1) not null constraint PK_PACKAGES2 primary key nonclustered,
...
begin
	...
	begin
		...
		begin
			...
			select @lastval = max(AG_ID) from a2agent.AGENTS where AG_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2agent.AGENTS", RESEED,  @lastval) with NO_INFOMSGS;
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(RTK_ID) from a2misc.RATE_KINDS where RTK_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2misc.RATE_KINDS", RESEED,  @lastval) with NO_INFOMSGS;
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(DC_ID) from a2entity.DISCOUNT_CARDS where DC_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2entity.DISCOUNT_CARDS", RESEED,  @lastval) with NO_INFOMSGS;
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(GR_ID) from a2misc.GROUPS where GR_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2misc.GROUPS", RESEED,  @lastval) with NO_INFOMSGS;
		end
		else
		begin
			insert into a2misc.GROUPS (GR_ID, GR_KIND) values (@id, @kind);
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(BA_ID) from a2agent.BANK_ACCOUNTS where BA_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2agent.BANK_ACCOUNTS", RESEED,  @lastval) with NO_INFOMSGS;
		end
		else
		begin
			insert into a2agent.BANK_ACCOUNTS (BA_ID, AG_ID, BA_ACC, BA_NAME, CRC_ID) values (@id, @agid, N'***REPL***', N'***REPL***', 980);
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(ENT_ID) from a2entity.ENTITIES where ENT_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2entity.ENTITIES", RESEED,  @lastval) with NO_INFOMSGS;
		end
		else
		begin
			insert into a2entity.ENTITIES (ENT_ID, ENT_KIND, ENT_P0, ENT_NAME) values (@id, N'REPL', 0, N'***REPL***');
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(ENT_ID) from a2entity.ENTITY_CLASS where EC_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2entity.ENTITY_CLASS", RESEED,  @lastval) with NO_INFOMSGS;
		end
		else
		begin
			insert into a2entity.ENTITY_CLASS(EC_ID, EC_KIND, EC_P0) values (@id, N'REPL', 0);
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(PL_ID) from a2entity.PRICE_LISTS where PL_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2entity.PRICE_LISTS", RESEED,  @lastval) with NO_INFOMSGS;
		end
		else
		begin
			insert into a2entity.PRICE_LISTS (PL_ID, PL_NAME) values (@id, N'***REPL***');
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(PK_ID) from a2entity.PRICE_KINDS where PK_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2entity.PRICE_KINDS", RESEED,  @lastval) with NO_INFOMSGS;
		end
		else
		begin
			insert into a2entity.PRICE_KINDS (PK_ID, PL_ID, PK_NAME, CRC_ID) values (@id, @plid, N'***REPL***', 980);
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(CT_ID) from a2doc.CONTRACTS where CT_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2doc.CONTRACTS", RESEED,  @lastval) with NO_INFOMSGS;
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(D_ID) from a2doc.DOCUMENTS where D_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2doc.DOCUMENTS", RESEED,  @lastval) with NO_INFOMSGS;
		end
		else
		begin
			insert into a2doc.DOCUMENTS (D_ID, D_KIND) values (@id, N'NULL');
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(DD_ID) from a2doc.DOC_DETAILS where DD_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2doc.DOC_DETAILS", RESEED,  @lastval) with NO_INFOMSGS;
		end
		else
		begin
			insert into a2doc.DOC_DETAILS (DD_ID, D_ID) values (@id, 0);
...
begin
	...
	begin
		...
		begin
			...
			declare @lastval bigint;
			select @lastval = max(S_ID) from a2jrn.SERIES where S_ID<a2sys.fn_makedbid(1, 0); 
			dbcc checkident ("a2jrn.SERIES", RESEED,  @lastval) with NO_INFOMSGS;
		end
		else
		begin
			insert into a2jrn.SERIES (S_ID, D_ID, DD_ID, ENT_ID, S_PRICE) values (@id, 0, 0, 0, 0.0);
