...
begin
	...
	if 1 = (select is_identity from sys.columns where name=N'MS_ID' and object_id=OBJECT_ID(N'a2agent.MARITAL_STATES'))
		exec a2repl.set_db_id_for_table N'a2agent.MARITAL_STATES', N'MS_ID', @dbid;	
		...
		begin
			insert into a2entity.ENTITIES 
				(ENT_ID,		   ENT_VOID, ENT_ACTIVE, ENT_MAIN, ENT_P0,		   B_ID,     V_ID,
				 ...
				 VT_ID,				 ENT_BITS, ENT_FLAG,   ENT_BASE, ENT_PACK,		 ENT_SITCODE) values
				(@Id,					 @Void,		 @Active,		 @Main,	   @ParentId,		 @BrandId, @VendorId,
				 ...
				 @VatId,			 @Bits,    @Flags,     @Base,    @Pack,			   @SitCode
				);
		...
		begin
			insert into a2entity.ENTITIES 
				(ENT_ID,		   ENT_VOID, ENT_ACTIVE, ENT_MAIN, ENT_P0,		   B_ID,     V_ID,
				 ...
				 ENT_CANCHILD, ENT_SIGN, UN_ID,      ENT_CAT,	 ENT_IMAGE,    ENT_LONG1,ENT_LONG2,
				 ENT_LONG3,		 ENT_GEN,	 ENT_SET,		 AG_ID,		 ENT_BARCODE,  CO_CODE,  ENT_ARTICLE, 
				 VT_ID,			   ENT_BITS, ENT_FLAG,  ENT_BASE,  ENT_PACK) values
				(@Id,					 @Void,		 @Active,		 @Main,	   @ParentId,		 @BrandId, @VendorId,
				 @Kind,				 @Name,		 @Tag,       @Memo,    @FullName,		 @Type,		 @SysId,
				 @CanChild,		 @Sign,	   @UnitId,		 @Cat,		 @Image,			 @Long1,   @Long2, 
				 @Long3,			 @Gen,		 @Set,			 @AgentId, @BarCode,     @CoCode,  @Article, 
				 @VatId,			 @Bits,    @Flags,     @Base,    @Pack
				);
		end
	end
end
...
------------------------------------------------
if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_SCHEMA=N'a2repl' and ROUTINE_NAME=N'set_all_sequences_client')
	drop procedure a2repl.set_all_sequences_client
go
------------------------------------------------
create procedure a2repl.set_all_sequences_client
as
begin
	...
	select @dbid = a2sys.fn_getdbid();
	set @minval = a2sys.fn_makedbid(@dbid, 1);


	-- a2sys
	select @max = isnull(max(ACL_PK) + 1, @minval) from a2sys.ACL where @dbid = a2sys.dbid2hp(ACL_PK)
	set @sql = N'alter sequence a2sys.SQ_ACL restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2sys.SQ_ACL:' + char(9) + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(LOG_ID) + 1, @minval) from a2sys.APP_LOG where @dbid = a2sys.dbid2hp(LOG_ID)
	set @sql = N'alter sequence a2sys.SQ_APP_LOG restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2sys.SQ_APP_LOG:' + char(9) + char(9) + cast(@max as nvarchar(32));


	-- a2agent
	select @max = isnull(max(AG_ID) + 1, @minval) from a2agent.AGENTS where @dbid = a2sys.dbid2hp(AG_ID)
	set @sql = N'alter sequence a2agent.SQ_AGENTS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_AGENTS:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(RR_ID) + 1, @minval) from a2agent.RL_REASONS where @dbid = a2sys.dbid2hp(RR_ID)
	set @sql = N'alter sequence a2agent.SQ_RL_REASONS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_RL_REASONS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(PHT_ID) + 1, @minval) from a2agent.PHONE_TYPES where @dbid = a2sys.dbid2hp(PHT_ID)
	set @sql = N'alter sequence a2agent.SQ_PHONE_TYPES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_PHONE_TYPES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(EMT_ID) + 1, @minval) from a2agent.EMAIL_TYPES where @dbid = a2sys.dbid2hp(EMT_ID)
	set @sql = N'alter sequence a2agent.SQ_EMAIL_TYPES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_EMAIL_TYPES:' + char(9) + cast(@max as nvarchar(32));
	
	select @max = isnull(max(ADDR_ID) + 1, @minval) from a2agent.AG_ADDRESSES where @dbid = a2sys.dbid2hp(ADDR_ID)
	set @sql = N'alter sequence a2agent.SQ_AG_ADDRESSES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_AG_ADDRESSES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(PH_ID) + 1, @minval) from a2agent.AG_PHONES where @dbid = a2sys.dbid2hp(PH_ID)
	set @sql = N'alter sequence a2agent.SQ_AG_PHONES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_AG_PHONES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(SH_ID) + 1, @minval) from a2agent.SCHEDULES where @dbid = a2sys.dbid2hp(SH_ID)
	set @sql = N'alter sequence a2agent.SQ_SCHEDULES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_SCHEDULES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(POS_ID) + 1, @minval) from a2agent.POSITIONS where @dbid = a2sys.dbid2hp(POS_ID)
	set @sql = N'alter sequence a2agent.SQ_POSITIONS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_POSITIONS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(STT_ID) + 1, @minval) from a2agent.STAFF_TABLES where @dbid = a2sys.dbid2hp(STT_ID)
	set @sql = N'alter sequence a2agent.SQ_STAFF_TABLES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_STAFF_TABLES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(BA_ID) + 1, @minval) from a2agent.BANK_ACCOUNTS where @dbid = a2sys.dbid2hp(BA_ID)
	set @sql = N'alter sequence a2agent.SQ_BANK_ACCOUNTS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_BANK_ACCOUNTS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(SCH_ID) + 1, @minval) from a2agent.SCHOOLS where @dbid = a2sys.dbid2hp(SCH_ID)
	set @sql = N'alter sequence a2agent.SQ_SCHOOLS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_SCHOOLS:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(PRS_ID) + 1, @minval) from a2agent.PROFESSIONS where @dbid = a2sys.dbid2hp(PRS_ID)
	set @sql = N'alter sequence a2agent.SQ_PROFESSIONS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_PROFESSIONS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(CA_ID) + 1, @minval) from a2agent.CASH_ACCOUNTS where @dbid = a2sys.dbid2hp(CA_ID)
	set @sql = N'alter sequence a2agent.SQ_CASH_ACCOUNTS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_CASH_ACCOUNTS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(AGU_ID) + 1, @minval) from a2agent.AG_USER where @dbid = a2sys.dbid2hp(AGU_ID)
	set @sql = N'alter sequence a2agent.SQ_AG_USER restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_AG_USER:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(AUG_ID) + 1, @minval) from a2agent.AG_USER_GROUPS where @dbid = a2sys.dbid2hp(AUG_ID)
	set @sql = N'alter sequence a2agent.SQ_AG_USER_GROUPS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_AG_USER_GROUPS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(STU_ID) + 1, @minval) from a2agent.STAFF_UNITS where @dbid = a2sys.dbid2hp(STU_ID)
	set @sql = N'alter sequence a2agent.SQ_STAFF_UNITS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_STAFF_UNITS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(MRS_ID) + 1, @minval) from a2agent.MARITAL_STATES where @dbid = a2sys.dbid2hp(MRS_ID)
	set @sql = N'alter sequence a2agent.SQ_MARITAL_STATES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_MARITAL_STATES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(SS_ID) + 1, @minval) from a2agent.SPEC_STAND_CODES where @dbid = a2sys.dbid2hp(SS_ID)
	set @sql = N'alter sequence a2agent.SQ_SPEC_STAND_CODES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_SPEC_STAND_CODES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(ADRT_ID) + 1, @minval) from a2agent.ADDR_TYPES where @dbid = a2sys.dbid2hp(ADRT_ID)
	set @sql = N'alter sequence a2agent.SQ_ADDR_TYPES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_ADDR_TYPES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(AGI_ID) + 1, @minval) from a2agent.AGINFO_TYPES where @dbid = a2sys.dbid2hp(AGI_ID)
	set @sql = N'alter sequence a2agent.SQ_AGINFO_TYPES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_AGINFO_TYPES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(EM_ID) + 1, @minval) from a2agent.AG_EMAILS where @dbid = a2sys.dbid2hp(EM_ID)
	set @sql = N'alter sequence a2agent.SQ_AG_EMAILS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_AG_EMAILS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(AI_ID) + 1, @minval) from a2agent.AG_INFOS where @dbid = a2sys.dbid2hp(AI_ID)
	set @sql = N'alter sequence a2agent.SQ_AG_INFOS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2agent.SQ_AG_INFOS:' + char(9) + cast(@max as nvarchar(32));

	-- a2doc
	select @max = isnull(max(D_ID) + 1, @minval) from a2doc.DOCUMENTS where @dbid = a2sys.dbid2hp(D_ID)
	set @sql = N'alter sequence a2doc.SQ_DOCUMENTS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2doc.SQ_DOCUMENTS:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DD_ID) + 1, @minval) from a2doc.DOC_DETAILS where @dbid = a2sys.dbid2hp(DD_ID)
	set @sql = N'alter sequence a2doc.SQ_DOC_DETAILS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2doc.SQ_DOC_DETAILS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DDP_ID) + 1, @minval) from a2doc.DOC_DETAILS_P where @dbid = a2sys.dbid2hp(DDP_ID)
	set @sql = N'alter sequence a2doc.SQ_DOC_DETAILS_P restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2doc.SQ_DOC_DETAILS_P:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(ST_ID) + 1, @minval) from a2doc.STATES where @dbid = a2sys.dbid2hp(ST_ID)
	set @sql = N'alter sequence a2doc.SQ_STATES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2doc.SQ_STATES:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DDS_ID) + 1, @minval) from a2doc.DOC_DETAILS_SERIES where @dbid = a2sys.dbid2hp(DDS_ID)
	set @sql = N'alter sequence a2doc.SQ_DOC_DETAILS_SERIES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2doc.SQ_DOC_DETAILS_SERIES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(CT_ID) + 1, @minval) from a2doc.CONTRACTS where @dbid = a2sys.dbid2hp(CT_ID)
	set @sql = N'alter sequence a2doc.SQ_CONTRACTS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2doc.SQ_CONTRACTS:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(CTD_ID) + 1, @minval) from a2doc.CONTRACT_DETAILS where @dbid = a2sys.dbid2hp(CTD_ID)
	set @sql = N'alter sequence a2doc.SQ_CONTRACT_DETAILS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2doc.SQ_CONTRACT_DETAILS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DDPS_ID) + 1, @minval) from a2doc.DOC_DETAILS_P_SUBROWS where @dbid = a2sys.dbid2hp(DDPS_ID)
	set @sql = N'alter sequence a2doc.SQ_DOC_DETAILS_P_SUBROWS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2doc.SQ_DOC_DETAILS_P_SUBROWS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DDPD_ID) + 1, @minval) from a2doc.DOC_DETAILS_P_DATA where @dbid = a2sys.dbid2hp(DDPD_ID)
	set @sql = N'alter sequence a2doc.SQ_DOC_DETAILS_P_DATA restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2doc.SQ_DOC_DETAILS_P_DATA:' + char(9) + cast(@max as nvarchar(32));

	-- a2jrn
	select @max = isnull(max(S_ID) + 1, @minval) from a2jrn.SERIES where @dbid = a2sys.dbid2hp(S_ID)
	set @sql = N'alter sequence a2jrn.SQ_SERIES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2jrn.SQ_SERIES:' + char(9) + char(9) + cast(@max as nvarchar(32));

	-- a2entity
	select @max = isnull(max(ENT_ID) + 1, @minval) from a2entity.ENTITIES where @dbid = a2sys.dbid2hp(ENT_ID) and ENT_ID<999999999
	set @sql = N'alter sequence a2entity.SQ_ENTITIES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_ENTITIES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(UN_ID) + 1, @minval) from a2entity.UNITS where @dbid = a2sys.dbid2hp(UN_ID)
	set @sql = N'alter sequence a2entity.SQ_UNITS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_UNITS:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(V_ID) + 1, @minval) from a2entity.VENDORS where @dbid = a2sys.dbid2hp(V_ID)
	set @sql = N'alter sequence a2entity.SQ_VENDORS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_VENDORS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(B_ID) + 1, @minval) from a2entity.BRANDS where @dbid = a2sys.dbid2hp(B_ID)
	set @sql = N'alter sequence a2entity.SQ_BRANDS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_BRANDS:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DC_ID) + 1, @minval) from a2entity.DISCOUNT_CARDS where @dbid = a2sys.dbid2hp(DC_ID)
	set @sql = N'alter sequence a2entity.SQ_DISCOUNT_CARDS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_DISCOUNT_CARDS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(PR_ID) + 1, @minval) from a2entity.PRICES where @dbid = a2sys.dbid2hp(PR_ID)
	set @sql = N'alter sequence a2entity.SQ_PRICES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_PRICES:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(PK_ID) + 1, @minval) from a2entity.PRICE_KINDS where @dbid = a2sys.dbid2hp(PK_ID)
	set @sql = N'alter sequence a2entity.SQ_PRICE_KINDS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_PRICE_KINDS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(PL_ID) + 1, @minval) from a2entity.PRICE_LISTS where @dbid = a2sys.dbid2hp(PL_ID)
	set @sql = N'alter sequence a2entity.SQ_PRICE_LISTS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_PRICE_LISTS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(EC_ID) + 1, @minval) from a2entity.ENTITY_CLASS where @dbid = a2sys.dbid2hp(EC_ID)
	set @sql = N'alter sequence a2entity.SQ_ENTITY_CLASS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_ENTITY_CLASS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DSV_ID) + 1, @minval) from a2entity.DISCOUNT_VALUES where @dbid = a2sys.dbid2hp(DSV_ID)
	set @sql = N'alter sequence a2entity.SQ_DISCOUNT_VALUES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_DISCOUNT_VALUES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DSVI_ID) + 1, @minval) from a2entity.DISCOUNT_VALUES_ITEMS where @dbid = a2sys.dbid2hp(DSVI_ID)
	set @sql = N'alter sequence a2entity.SQ_DISCOUNT_VALUES_ITEMS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_DISCOUNT_VALUES_ITEMS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DCS_ID) + 1, @minval) from a2entity.DISCOUNT_CARD_CLASSES where @dbid = a2sys.dbid2hp(DCS_ID)
	set @sql = N'alter sequence a2entity.SQ_DISCOUNT_CARD_CLASSES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_DISCOUNT_CARD_CLASSES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(DS_ID) + 1, @minval) from a2entity.DISCOUNTS where @dbid = a2sys.dbid2hp(DS_ID)
	set @sql = N'alter sequence a2entity.SQ_DISCOUNTS restart with ' + cast(@max as nvarchar(64));
	exec sp_executesql @sql;
	print N'a2entity.SQ_DISCOUNTS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(EG_ID) + 1, @minval) from a2entity.ENTITY_GROUPS where @dbid = a2sys.dbid2hp(EG_ID)
	set @sql = N'alter sequence a2entity.SQ_ENTITY_GROUPS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_ENTITY_GROUPS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(ES_ID) + 1, @minval) from a2entity.ENTITY_SETS where @dbid = a2sys.dbid2hp(ES_ID)
	set @sql = N'alter sequence a2entity.SQ_ENTITY_SETS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_ENTITY_SETS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(RP_ID) + 1, @minval) from a2entity.RECIPES where @dbid = a2sys.dbid2hp(RP_ID)
	set @sql = N'alter sequence a2entity.SQ_RECIPES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_RECIPES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(RPI_ID) + 1, @minval) from a2entity.RECIPE_ITEMS where @dbid = a2sys.dbid2hp(RPI_ID)
	set @sql = N'alter sequence a2entity.SQ_RECIPE_ITEMS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_RECIPE_ITEMS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(ENTC_ID) + 1, @minval) from a2entity.ENT_CODES where @dbid = a2sys.dbid2hp(ENTC_ID)
	set @sql = N'alter sequence a2entity.SQ_ENT_CODES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_ENT_CODES:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(ENTSC_ID) + 1, @minval) from a2entity.ENT_SUPPLIER_CODES where @dbid = a2sys.dbid2hp(ENTSC_ID)
	set @sql = N'alter sequence a2entity.SQ_ENT_SUPPLIER_CODES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2entity.SQ_ENT_SUPPLIER_CODES:' + char(9) + cast(@max as nvarchar(32));

	-- a2misc
	select @max = isnull(max(GR_ID) + 1, @minval) from a2misc.GROUPS where @dbid = a2sys.dbid2hp(GR_ID)
	set @sql = N'alter sequence a2misc.SQ_GROUPS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2misc.SQ_GROUPS:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = max(UT_ID) + 1 from a2misc.UTABLES where @dbid = a2sys.dbid2hp(UT_ID)
	set @sql = N'alter sequence a2misc.SQ_UTABLES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;

	select @max = max(RT_ID) + 1 from a2misc.RATES where @dbid = a2sys.dbid2hp(RT_ID)
	set @sql = N'alter sequence a2misc.SQ_RATES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;

	select @max = max(UTR_ID) + 1 from a2misc.UT_RANGES where @dbid = a2sys.dbid2hp(UTR_ID)
	set @sql = N'alter sequence a2misc.SQ_UT_RANGES restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;

	select @max = isnull(max(AGN_ID) + 1, @minval) from a2misc.AGENDAS where @dbid = a2sys.dbid2hp(AGN_ID)
	set @sql = N'alter sequence a2misc.SQ_AGENDAS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2misc.SQ_AGENDAS:' + char(9) + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(APN_ID) + 1, @minval) from a2misc.APPOINTMENTS where @dbid = a2sys.dbid2hp(APN_ID)
	set @sql = N'alter sequence a2misc.SQ_APPOINTMENTS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2misc.SQ_APPOINTMENTS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(RTK_ID) + 1, @minval) from a2misc.RATE_KINDS where @dbid = a2sys.dbid2hp(RTK_ID)
	set @sql = N'alter sequence a2misc.SQ_RATE_KINDS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2misc.SQ_RATE_KINDS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(PRM_ID) + 1, @minval) from a2misc.DB_PARAMS where @dbid = a2sys.dbid2hp(PRM_ID)
	set @sql = N'alter sequence a2misc.SQ_DB_PARAMS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2misc.SQ_DB_PARAMS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(TPRM_ID) + 1, @minval) from a2misc.TREE_PARAMS where @dbid = a2sys.dbid2hp(TPRM_ID)
	set @sql = N'alter sequence a2misc.SQ_TREE_PARAMS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2misc.SQ_TREE_PARAMS:' + char(9) + cast(@max as nvarchar(32));

	-- a2budget
	select @max = isnull(max(CC_ID) + 1, @minval) from a2budget.COST_CENTERS where @dbid = a2sys.dbid2hp(CC_ID)
	set @sql = N'alter sequence a2budget.SQ_COST_CENTERS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2budget.SQ_COST_CENTERS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(BD_ID) + 1, @minval) from a2budget.BUDGETS where @dbid = a2sys.dbid2hp(BD_ID)
	set @sql = N'alter sequence a2budget.SQ_BUDGETS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2budget.SQ_BUDGETS:' + char(9) + cast(@max as nvarchar(32));

	select @max = isnull(max(BI_ID) + 1, @minval) from a2budget.BUDGET_ITEMS where @dbid = a2sys.dbid2hp(BI_ID)
	set @sql = N'alter sequence a2budget.SQ_BUDGET_ITEMS restart with ' + cast(@max as nvarchar(32));
	exec sp_executesql @sql;
	print N'a2budget.SQ_BUDGET_ITEMS:' + char(9) + cast(@max as nvarchar(32));
end
go
------------------------------------------------
...
go
------------------------------------------------
-- повторная установка sequences
exec a2repl.set_all_sequences_client
go
